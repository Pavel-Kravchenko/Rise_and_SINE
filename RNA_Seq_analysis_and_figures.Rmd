---
title: "The main code for 'Mechanisms of zygotic genome activation in totipotent mammalian embryos'. RNA-seq analysis and Figures 3, 4, S1A"
output:
  html_document:
    df_print: paged
...
home = "/Volumes/pool-toti-bioinfo/bioinfo/Pavel.Kravchenko"
tables = "/Users/pavel/Desktop/Rise_and_SINE/tables/RNA_seq_secondary_analysis_DESeq_tables"
...
dev.off()
```
# Plotting the Figure_S1A using nVennR_gene_list_polyA with plotVenn
```{r}
nVennR_gene_list_polyA <- list(
     "B6D2F1 pure (N=1)" = unique(toupper(annotatedRes_Chen$SYMBOL)),
     "C57BL/6 x mix (N=3)" = unique(Reduce(intersect,list(unique(toupper(annotatedRes_Gassler_B6xCAST$SYMBOL)),
...
# Plotting Pan lists overlap with TF-controlled genes. Figure 4B, 4C
```{r}
Dux_KO <- file.path("./Figures/Figure_4/41588_2019_418_MOESM3_ESM.xlsx")
Dux_KO <- read_xlsx(Dux_KO, sheet=2)
Dux_KO = Dux_KO[which(Dux_KO$...17 == "down-regulated"),]
Dux_KO = unique(toupper(na.omit(Dux_KO[["...1"]]))) #[-c(1,2)]
write.csv(Dux_KO, "./Figures/Figure_4/Dux_KO_ZGA_genes.csv", row.names=FALSE)

res_mm = read.delim("./Figures/Figure_4/science.abn7478_table_s6.csv", sep = ";")
Nr5a2_KD = unique(toupper(na.omit(unique(res_mm[which(res_mm$State == "DOWN"),]$gene_id))))
write.csv(Nr5a2_KD, "./Figures/Figure_4/Nr5a2_KD_ZGA_genes.csv", row.names=FALSE)

res_mm = read.delim("./Figures/Figure_4/science.abn7478_table_s5.csv", sep = ";")
Nr5a2_SR1848 = unique(toupper(na.omit(unique(res_mm[which(res_mm$State == "DOWN"),]$gene_id))))
write.csv(Nr5a2_SR1848, "./Figures/Figure_4/Nr5a2_SR1848_ZGA_genes.csv", row.names=FALSE)

Nr5a2_total = unique(toupper(na.omit(unique(Reduce(c,list(unique(toupper(Nr5a2_SR1848)), unique(Nr5a2_KD)))))))
write.csv(Nr5a2_total, "./Figures/Figure_4/Nr5a2_total_ZGA_genes.csv", row.names=FALSE)

res_mm = read.delim("./Figures/Figure_4/41586_2023_6428_MOESM3_ESM.csv", sep = ";")
Obox_KO = unique(toupper(na.omit(unique(res_mm[which(res_mm$Group == "down-regulated"),]$gene.name))))
write.csv(Obox_KO, "./Figures/Figure_4/Obox_KO_ZGA_genes.csv", row.names=FALSE)


df2 <- data.frame(gene=unique(unlist(nVennR_gene_list_polyA)))
write.csv(df2, "./Figures/Figure_4/ZGA_genes_summary_table.csv", row.names=FALSE)
...
# length(unique(Reduce(intersect,list(unique(toupper(Nr5a2_SR1848)), unique(toupper(ZGA_genes_all)))))) #908


pV = plotVenn(gene_list_polyA_inter_uni, outFile="./Figures/Figure_4/Figure_4B/Figure_4B.svg")
...
pdf("./Figures/Figure_4/Figure_4C/Figure_4C_ylim_1500.pdf")
set_size(8, 5)
size = get_size_mode('exclusive_intersection')
...
pdf("./Figures/Figure_4/Figure_4C/Figure_4C_ylim_5000.pdf")
set_size(8, 5)
size = get_size_mode('exclusive_intersection')
ComplexUpset::upset(fromList(gene_list_polyA_inter_uni), 
                    names(gene_list_polyA_inter_uni), 
...
pdf("./Figures/Figure_4/Figure_4A/Figure_4A.pdf")
