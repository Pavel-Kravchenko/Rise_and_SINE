...
```{r}
Dux_KO <- file.path("./Figures/Figure_S1/Figure_1SB/41588_2019_418_MOESM3_ESM.xlsx")
Dux_KO <- read_xlsx(Dux_KO, sheet=2)
Dux_KO = Dux_KO[which(Dux_KO$...17 == "down-regulated"),]
Dux_KO = unique(toupper(na.omit(Dux_KO[["...1"]]))) #[-c(1,2)]
write.csv(Dux_KO, "./Figures/Figure_S1/Figure_1SB/Dux_KO_ZGA_genes.csv", row.names=FALSE)

res_mm = read.delim("./Figures/Figure_S1/Figure_1SB/science.abn7478_table_s6.csv", sep = ";")
Nr5a2_KD = unique(toupper(na.omit(unique(res_mm[which(res_mm$State == "DOWN"),]$gene_id))))
write.csv(Nr5a2_KD, "./Figures/Figure_S1/Figure_1SB/Nr5a2_KD_ZGA_genes.csv", row.names=FALSE)

res_mm = read.delim("./Figures/Figure_S1/Figure_1SB/science.abn7478_table_s5.csv", sep = ";")
Nr5a2_SR1848 = unique(toupper(na.omit(unique(res_mm[which(res_mm$State == "DOWN"),]$gene_id))))
write.csv(Nr5a2_SR1848, "./Figures/Figure_S1/Figure_1SB/Nr5a2_SR1848_ZGA_genes.csv", row.names=FALSE)

Nr5a2_total = unique(toupper(na.omit(unique(Reduce(c,list(unique(toupper(Nr5a2_SR1848)), unique(Nr5a2_KD)))))))
write.csv(Nr5a2_total, "./Figures/Figure_S1/Figure_1SB/Nr5a2_total_ZGA_genes.csv", row.names=FALSE)

res_mm = read.delim("./Figures/Figure_S1/Figure_1SB/41586_2023_6428_MOESM3_ESM.csv", sep = ";")
Obox_KO = unique(toupper(na.omit(unique(res_mm[which(res_mm$Group == "down-regulated"),]$gene.name))))
write.csv(Obox_KO, "./Figures/Figure_S1/Figure_1SB/Obox_KO_ZGA_genes.csv", row.names=FALSE)

res_mm = read.csv2("./Figures/Figure_S1/Figure_1SB/NIHMS1533688-supplement-14.csv")
Nfya_KO = unique(res_mm[res_mm$log2fc_2.Cell_NfyaKD > log2(1.5), ]$gene)


df2 <- data.frame(gene=unique(unlist(nVennR_gene_list_polyA)))
write.csv(df2, "./Figures/Figure_S1/Figure_1SB/ZGA_genes_summary_table.csv", row.names=FALSE)
...
pdf("./Figures/Figure_S1/Figure_1SB/Figure_1SB_ylim_1500.pdf")
set_size(8, 5)
size = get_size_mode('exclusive_intersection')
...
pdf("./Figures/Figure_S1/Figure_1SB/Figure_1SB_ylim_5000.pdf")
set_size(8, 5)
size = get_size_mode('exclusive_intersection')
ComplexUpset::upset(fromList(gene_list_polyA_inter_uni), 
                    names(gene_list_polyA_inter_uni), 
