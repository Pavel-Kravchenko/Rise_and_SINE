...
#SBATCH --error=[full path]/log/TrimGalore%A.%a.err
#SBATCH --output=[full path]/log/TrimGalore%A.%a.out
...
do
    ...
    then
        ...
        echo $(sed -n ${index}p ${BASIC_DIR}/scripts/kallisto_cmd1.sh)
        eval $(sed -n ${index}p ${BASIC_DIR}/scripts/kallisto_cmd1.sh) || exit 100
    fi
    step=$(($step+1))
done
