...
PK_stable = read.csv('/Volumes/pool-totipotency/Pavel/projects/Motif_search_before_minor_ZGA_genes/Data/minor_ZGA_datasets/minor_ZGA_stable_PK.csv', header = TRUE, sep=",")[["x"]]
PK_surge = read.csv('/Volumes/pool-totipotency/Pavel/projects/Motif_search_before_minor_ZGA_genes/Data/minor_ZGA_datasets/minor_ZGA_surge_PK.csv', header = TRUE, sep=",")[["x"]]

IG_stable = read.csv('/Volumes/pool-totipotency/Pavel/projects/Motif_search_before_minor_ZGA_genes/Data/minor_ZGA_datasets/minor_ZGA_probes.csv', header = TRUE, sep=",")
IG_stable = IG_stable[which(IG_stable["category"] == "stable"),][["gene_id"]]
IG_surge = read.csv('/Volumes/pool-totipotency/Pavel/projects/Motif_search_before_minor_ZGA_genes/Data/minor_ZGA_datasets/minor_ZGA_probes.csv', header = TRUE, sep=",")
IG_surge = IG_surge[which(IG_surge["category"] == "surge"),][["gene_id"]]

Hu_stable = read.csv('/Volumes/pool-totipotency/Pavel/projects/Motif_search_before_minor_ZGA_genes/Data/minor_ZGA_datasets/minorZGA_Hu_Cluster4.csv', header = F, sep=",")[["V1"]]
Hu_surge = read.csv('/Volumes/pool-totipotency/Pavel/projects/Motif_search_before_minor_ZGA_genes/Data/minor_ZGA_datasets/minorZGA_Hu_Cluster2.csv', header = F, sep=",")[["V1"]]

DBTMEE_stable = read.csv('/Volumes/pool-totipotency/Pavel/projects/Motif_search_before_minor_ZGA_genes/Data/minor_ZGA_datasets/minorZGA_DBTMEE.csv', header = F, sep=",")[["V1"]]
DBTMEE_surge = read.csv('/Volumes/pool-totipotency/Pavel/projects/Motif_search_before_minor_ZGA_genes/Data/minor_ZGA_datasets/minorZGA_DBTMEE.csv', header = F, sep=",")[["V1"]]

UCSC_stable = read.csv('/Volumes/pool-totipotency/Pavel/projects/Motif_search_before_minor_ZGA_genes/Data/minor_ZGA_datasets/embj2022112012_e2C.csv', header = F, sep=",")[["V1"]]
UCSC_surge = read.csv('/Volumes/pool-totipotency/Pavel/projects/Motif_search_before_minor_ZGA_genes/Data/minor_ZGA_datasets/embj2022112012_Z.csv', header = F, sep=",")[["V1"]]

Abe_stable = read.csv('/Volumes/pool-totipotency/Pavel/projects/Motif_search_before_minor_ZGA_genes/Data/minor_ZGA_datasets/minorZGA_Abe_Cluster4.csv', header = F, sep=",")[["V1"]]
Abe_surge = read.csv('/Volumes/pool-totipotency/Pavel/projects/Motif_search_before_minor_ZGA_genes/Data/minor_ZGA_datasets/minorZGA_Abe_Cluster5.csv', header = F, sep=",")[["V1"]]
...
```{r}
gene_list <- list(
  #"PK_stable" = unique(toupper(PK_stable)),
  "ZGA_FISH_stable" = unique(toupper(IG_stable)),
  "Hu_stable" = unique(toupper(Hu_stable)),
  "DBTMEE_stable" = unique(toupper(DBTMEE_stable)),
  "UCSC_stable" = unique(toupper(UCSC_stable)),
  "Abe_stable" = unique(toupper(Abe_stable)))


p = plot(euler(gene_list, shape="ellipse"), quantities = TRUE, legend = TRUE)
print(p)
pV = plotVenn(gene_list, outFile="./figures/minor.svg")
...
ComplexUpset::upset(fromList(gene_list), #[apply(fromList(nVennR_gene_list_polyA_total),1,sum) > 3, ], 
                    names(gene_list), 
                    width_ratio=0.1, 
...
```{r}
gene_list <- list(
  "ZGA_FISH_surge" = unique(toupper(IG_surge)),
  "Hu_surge" = unique(toupper(Hu_surge)),
  "DBTMEE_surge" = unique(toupper(DBTMEE_surge)),
  "UCSC_surge" = unique(toupper(UCSC_surge)),
  "Abe_surge" = unique(toupper(Abe_surge)))
...
Nr5a2_SR1848 = unique(res_mm[which(res_mm$State == "DOWN"),]$gene_id)

Nr5a2_total = unique(Reduce(c,list(unique(toupper(Nr5a2_SR1848)), unique(Nr5a2_KD))))
...
#res_mm = read.csv2("./tables/Nfya_NIHMS1533688-supplement-14.csv")
#Nfya_KO = unique(res_mm[res_mm$log2fc_2.Cell_NfyaKD > log2(1.5), ]$gene)

#res_mm = read.csv2("./tables/res_mm_C88RF_20th_percentile_cleaned_technical_and_bio.csv")
#Esrrb_KD = unique(res_mm[res_mm$log2FoldChange > log2(1.5) & res_mm$padj < 0.05, ]$gene)
...
gene_list_polyA_inter_uni <- list(
  ...
  #"Esrrb regulated" = unique(toupper(Esrrb_KD)),
  #"Nfya regulated" = unique(toupper(Nfya_KO)),
...
#length(unique(Reduce(intersect,list(unique(toupper(Nfya_KO)), unique(toupper(ZGA_genes_all)))))) #14
#length(unique(Reduce(intersect,list(unique(toupper(Esrrb_KD)), unique(toupper(ZGA_genes_all)))))) # 44
...
#pV = plotVenn(gene_list_polyA_inter_uni, outFile="./figures/nVennR_NRs_Dux_Obox_Pan_ZGA_gene_list_polyA.svg")
...
write.csv(pc[pc$cnt > 2, ], "./Pan_1_degree_ZGA_genes.csv", row.names=FALSE)
write.csv(pc[pc$cnt > 1, ], "./Pan_2_degree_ZGA_genes.csv", row.names=FALSE)
write.csv(pc[pc$cnt > 0, ], "./Pan_3_degree_ZGA_genes.csv", row.names=FALSE)
write.csv(pc[pc$cnt == 4, ], "./Core_ZGA_genes.csv", row.names=FALSE)
write.csv(pc, "./Complete_ZGA_genes.csv", row.names=FALSE)
...
#unique(Reduce(intersect,list(unique(toupper(Nfya_KO)), unique(toupper(ZGA_genes_all)))))
unique(Reduce(intersect,list(unique(toupper(Esrrb_KD)), unique(toupper(ZGA_genes_all)))))
